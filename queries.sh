...
echo "$($PSQL "select t.name from teams as t left join games as g1 on t.team_id=g1.winner_id left join games as g2 on t.team_id=g2.opponent_id where (g1.year=2014 and g1.round='Eighth-Final') or (g2.year=2014 and g2.round='Eighth-Final') group by t.name order by t.name")"
